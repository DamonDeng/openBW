...
(async function () {
  ...
  function gameCard(g, kind) {
    ...
    // Observing a running game happens in the native Qt lobby app
    // (simsc_desktop). The browser-hosted WASM observer that used to
    // live here was retired 2026-08-09 -- see
    // docs/wasm_observer_deprecated.md for the history and reasons.
    // The web UI still lists games and manages their lifecycle;
    // attendees open games via simsc_desktop.
    let actions = '';
    if (kind === 'invitation') {
      actions = `
