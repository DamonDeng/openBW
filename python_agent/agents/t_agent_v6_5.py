...
def race_catalogs(race: str):
    ...
    raise SystemExit(f"[t_v6_5] race={race} not supported (this is the "
...
def verify_pending(pending: dict, obs: dict, stats: Stats,
    ...
    for key, p in pending.items():
        ...
        if cur_count > p.pre_count:
            ...
            print(f"[t_v6_5] TOOK  {p.label():48s} "
        ...
        if age >= grace_frames:
            ...
            if n == 1 or n % 5 == 0:
                print(f"[t_v6_5] REJECT {p.label():48s} after {age}f. n={n}. "
        ...
        if need > 0:
            ...
            for w in pool[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  gather-gas error: {e}")
    ...
    if mfs:
        for w in wu:
            ...
            try:
                ...
                print(f"[t_v6_5]  gather-min error: {e}")
...
async def phase_scout(c: Client, obs: dict, worker_type: int,
    ...
    for wid in list(scouts.keys()):
        if wid not in live_ids:
            print(f"[t_v6_5]  SCOUT worker {wid} died; unassigning")
            scouts.pop(wid, None)
    ...
    for mode, target_n in target_by_mode.items():
        ...
        for i in range(min(need, len(pool))):
            ...
            print(f"[t_v6_5]  SCOUT worker {w['unit_id']} mode={mode} "
    ...
    def next_wp_idx(sc: Scout, n: int) -> int:
        ...
        if len(sc.blacklist) >= n:
            print(f"[t_v6_5]  SCOUT {sc.worker_id} [{sc.mode}] all "
    ...
    for wid, sc in list(scouts.items()):
        ...
        print(f"[t_v6_5/SCOUT] {wid}[{sc.mode}] wp={sc.waypoint_idx} "
        ...
        if d < ARRIVE_RADIUS:
            ...
            print(f"[t_v6_5]  SCOUT {wid} [{sc.mode}] ARRIVED @{target}; "
                  f"next wp {new_idx} {wps[new_idx]}")
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
            except Exception as e:
                print(f"[t_v6_5]  scout move error: {e}")
            continue
        ...
        if stuck_reason is not None:
            print(f"[t_v6_5]  SCOUT {wid} [{sc.mode}] STUCK near "
            ...
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
                print(f"[t_v6_5]  SCOUT {wid} [{sc.mode}] -> wp {new_idx} "
                      f"{nxt} (blacklist size {len(sc.blacklist)})")
            except Exception as e:
                print(f"[t_v6_5]  scout skip-move error: {e}")
        else:
            ...
            if order_name_str not in ("Move", "MoveToAttack",
                ...
                try:
                    ...
                    print(f"[t_v6_5]  scout re-move error: {e}")
    ...
    for e in obs.get("enemies", []):
        if e.get("building") and e["unit_id"] not in known_enemies:
            ...
            print(f"[t_v6_5]  SCOUT SPOTTED enemy building "
    ...
    for n in obs.get("neutrals", []):
        ...
        if n["type"] in (176, 177, 178, 188):
            ...
            if dist_home > 1500:
                ...
                print(f"[t_v6_5]  SCOUT SPOTTED {kind} @({n['x']},{n['y']}) "
...
async def try_expand(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_5]  expand find_placement error: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        print(f"[t_v6_5]  EXPAND: no placement near cluster ({cx},{cy})")
        return None
    spot = spots[0]
    ...
    try:
        ...
        print(f"[t_v6_5]  expand build cmd error: {e}")
        return None

    pending_expansion_pts.add((cx, cy))
    print(f"[t_v6_5] FIRE  EXPAND CC @cluster ({cx},{cy}) "
...
async def try_build(c: Client, obs: dict, spec: BuildingSpec,
    ...
    try:
        ...
        print(f"[t_v6_5]  find_placement error {unit_type_name(spec.type_id)}: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        return None
    ...
    if spec.anchor != "geyser":
        ...
        for s in spots:
            ...
            if not in_mining_corridor(sx, sy, units, neutrals, main_type):
                keep.append(s)
        if not keep:
            print(f"[t_v6_5]  build {unit_type_name(spec.type_id)}: "
    ...
    try:
        ...
        print(f"[t_v6_5]  build cmd error {unit_type_name(spec.type_id)}: {e}")
        return None
    return Pending(
...
async def try_train_worker(c, obs, worker_type, main_type, cost_min,
    ...
    try:
        ...
        print(f"[t_v6_5]  train worker error: {e}")
        return None
...
async def try_train_unit(c, obs, spec: UnitSpec) -> Pending | None:
    ...
    try:
        ...
        print(f"[t_v6_5]  train unit error: {e}")
        return None
...
async def try_build_defense_structure(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_5]  defense find_placement error "
    ...
    try:
        ...
        print(f"[t_v6_5]  defense build cmd error "
              f"{unit_type_name(structure_type)}: {e}")
        return None
    return Pending(
...
async def phase_defense_line(c: Client, obs: dict,
    ...
    if (bunker_c + bunker_ip < bunker_target
        ...
        if p is not None:
            ...
            print(f"[t_v6_5] FIRE  build:Bunker "
    ...
    if (eb_c > 0
        ...
        if p is not None:
            ...
            pending_workers.add(p.worker_id)
            print(f"[t_v6_5] FIRE  build:Missile_Turret "
    ...
    for tank in units:
        ...
        try:
            ...
            print(f"[t_v6_5]  tank-arc move error: {e}")
    ...
    if bunkers:
        for m in units:
            ...
            try:
                ...
                print(f"[t_v6_5]  marine-ring move error: {e}")
    ...
    for cc in expansions:
        ...
        if g["bunker"] == -1:
            ...
            if near_bunkers:
                ...
                print(f"[t_v6_5]  GARRISON cc{cc_id}: adopted "
                      f"bunker {g['bunker']}")
        ...
        if len(g["marines"]) < 2:
            ...
            for m in avail_marines[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  GARRISON cc{cc_id}: assign "
                          f"marine {m['unit_id']} -> ({dst_x},{dst_y})")
                except Exception as e:
                    print(f"[t_v6_5]  garrison-marine move error: {e}")
        ...
        if g["tank"] == -1:
            ...
            if avail_tanks:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  garrison-tank move error: {e}")
...
async def try_upgrade(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_5] FIRE  {spec.kind}:{spec.label} @{src['unit_id']} "
              f"cost={spec.cost_min}/{spec.cost_gas}")
        return True
    except Exception as e:
        print(f"[t_v6_5]  {spec.kind} error: {e}")
        return False
...
async def run(c: Client, interval_sec: float,
    ...
    print(f"[t_v6_5] connected slot={c.welcome.slot} "
          f"frame={c.welcome.current_frame}")
    print(f"[t_v6_5] turtle config: "
    ...
    while True:
        ...
        if race is None:
            ...
            print(f"[t_v6_5] race={race} home=({home_x},{home_y}) "
                  f"map={map_w}x{map_h}")
            print(f"[t_v6_5] radial wps: {waypoints_by_mode['radial']}")
            print(f"[t_v6_5] zscan wps: {len(waypoints_by_mode['zscan'])} points "
            ...
            if exp_key in pending_expansion_cluster and exp_key not in pending:
                ...
                if nx_now <= len(own_of_type(units, main_type, only_complete=False)) - 1:
                    ...
                    if n >= EXPANSION_REJECT_THRESHOLD and cluster_pt not in cluster_blacklist:
                        ...
                        print(f"[t_v6_5]  EXPAND BLACKLIST cluster "
                              f"{cluster_pt}: {n} REJECTs in a row. "
                              f"Won't try again this game.")
                    else:
                        print(f"[t_v6_5]  EXPAND REJECT tracked for "
        ...
        for spec in catalog_upgrades:
            ...
            if spec.kind == "upgrade":
                ...
                if lvl > 0:
                    ...
                    print(f"[t_v6_5] TOOK  upgrade:{spec.label} "
                          f"(level {lvl})")
            else:  # research
                if spec.enum_id in obs_tech:
                    completed_upgrades.add(key)
                    print(f"[t_v6_5] TOOK  research:{spec.label}")
        ...
        if allow_advance:
            ...
            print(f"[t_v6_5]  DEFENSE step advanced to {defense_step:.2f} "
            ...
            for tank in units:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  rotate-unsiege error: {e}")
            if released > 0:
                print(f"[t_v6_5]  ROTATE unsiege parity={tank_slot_next_release}: "
        ...
        for pu in units:
            ...
            if is_repairable(pu):
                damaged_mech += 1
        print(f"[t_v6_5] f={frame} min={r['minerals']} gas={r['gas']} "
                ...
                if cands:
                    ...
                    try:
                        ...
                        if spots:
                            ...
                            print(f"[t_v6_5] FIRE  build:Supply_Depot "
                                  f"({pyl_total2 + 1}/{pylon_target}) "
                                  f"anchor={anchor_pt}")
                        else:
                            budget["min"] += 100  # refund
                    except Exception as e:
                        print(f"[t_v6_5]  supply-depot fire error: {e}")
                        budget["min"] += 100
                else:
                    budget["min"] += 100
        ...
        if n_workers < worker_target:
            ...
            for cc in completed_ccs:
                ...
                if not reserve(worker_train_min, 0):
                    if frame % 500 == 0:
                        print(f"[t_v6_5]  WORKER-STALL: want SCV "
                ...
                if result is not None:
                    ...
                    print(f"[t_v6_5] FIRE  train:SCV ({n_workers + 1}/{worker_target}) "
                          f"@cc{cc_id}")
                else:
        ...
        if opening_complete and not getattr(run, "_opening_logged", False):
            print(f"[t_v6_5] OPENING COMPLETE @frame {frame}: "
        ...
        if (opening_complete
            ...
            if reserve(100, 0):
                ...
                if p is not None:
                    ...
                    pending_workers.add(p.worker_id)
                    print(f"[t_v6_5] FIRE  build:Refinery")
                else:
                    budget["min"] += 100
        ...
        if depleted_bases > 0 and frame % 500 == 0:
            print(f"[t_v6_5]  DEPLETION detected: {depleted_bases} base(s) "
        ...
        for spec in catalog_buildings:
            ...
            if p is not None:
                pending[key] = p
                pending_workers.add(p.worker_id)
                print(f"[t_v6_5] FIRE  build:{unit_type_name(spec.type_id)}")
        ...
        for parent_type, addon_type, amin, agas, aname in _ADDON_ATTACHMENTS:
            ...
            for parent in own_of_type(units, parent_type):
                ...
                try:
                    ...
                    print(f"[t_v6_5] FIRE  addon:{aname} on "
                    ...
                    print(f"[t_v6_5]  addon fire error: {e}")
        ...
        for spec in catalog_units:
            ...
            if p is not None:
                pending[key] = p
                print(f"[t_v6_5] FIRE  train:{unit_type_name(spec.type_id)} "
        ...
        for bld in units:
            ...
            if bld.get("flying") and bld["unit_id"] in addon_lifted:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  land find_placement error: {e}")
                    continue
            ...
            try:
                ...
                print(f"[t_v6_5] FIRE  lift Terran_Factory "
                      f"{bld['unit_id']} for addon relocation "
                      f"(retries exhausted)")
                lifted = True
            except Exception as e:
                print(f"[t_v6_5]  addon-lift error: {e}")
            ...
            for tank in units:
                ...
                if should_siege:
                    try:
                        ...
                        print(f"[t_v6_5] FIRE  siege tank={tank['unit_id']} "
                              f"({reason})")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_5]  siege error: {e}")
                    ...
                    try:
                        await c.unsiege(unit_id=tank["unit_id"])
                        print(f"[t_v6_5] FIRE  unsiege tank={tank['unit_id']} "
                              f"(no enemy within {UNSIEGE_RANGE_PX}px, "
                              f"off slot)")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_5]  unsiege error: {e}")
        ...
        if _TECH_SPIDER_MINES in obs_tech and tgt_x is not None:
            for vult in units:
                ...
                try:
                    ...
                    print(f"[t_v6_5] FIRE  place_mine vulture={vid} "
                          f"@({mx},{my}) drop {drop_idx+1}/3")
                    mine_fires += 1
                except Exception as e:
                    print(f"[t_v6_5]  place_mine error: {e}")
        ...
        if not move_done:
            ...
            if idle:
                ...
                try:
                    ...
                    print(f"[t_v6_5]  cover move error: {e}")
        if move_done and not stop_done:
            ...
            if cands:
                try:
                    ...
                    print(f"[t_v6_5]  cover stop error: {e}")
...
def entrypoint() -> None:
    p = argparse.ArgumentParser(
        prog="python3 -m python_agent.agents.t_agent_v6_5",
        description="Terran v6_5: v6_4 + Factory placement further "
                    "from CC (Machine Shop room) + sustained SCV "
                    "training past opening. Load-into-bunker pass "
                    "reverted to ring-patrol until server exposes "
                    "transport state.")
