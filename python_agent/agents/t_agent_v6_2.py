...
def race_catalogs(race: str):
    ...
    raise SystemExit(f"[t_v6_2] race={race} not supported (this is the "
...
def verify_pending(pending: dict, obs: dict, stats: Stats,
    ...
    for key, p in pending.items():
        ...
        if cur_count > p.pre_count:
            ...
            print(f"[t_v6_2] TOOK  {p.label():48s} "
        ...
        if age >= grace_frames:
            ...
            if n == 1 or n % 5 == 0:
                print(f"[t_v6_2] REJECT {p.label():48s} after {age}f. n={n}. "
        ...
        if need > 0:
            ...
            for w in pool[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6_2]  gather-gas error: {e}")
    ...
    if mfs:
        for w in wu:
            ...
            try:
                ...
                print(f"[t_v6_2]  gather-min error: {e}")
...
async def phase_scout(c: Client, obs: dict, worker_type: int,
    ...
    for wid in list(scouts.keys()):
        if wid not in live_ids:
            print(f"[t_v6_2]  SCOUT worker {wid} died; unassigning")
            scouts.pop(wid, None)
    ...
    for mode, target_n in target_by_mode.items():
        ...
        for i in range(min(need, len(pool))):
            ...
            print(f"[t_v6_2]  SCOUT worker {w['unit_id']} mode={mode} "
    ...
    def next_wp_idx(sc: Scout, n: int) -> int:
        ...
        if len(sc.blacklist) >= n:
            print(f"[t_v6_2]  SCOUT {sc.worker_id} [{sc.mode}] all "
    ...
    for wid, sc in list(scouts.items()):
        ...
        print(f"[t_v6_2/SCOUT] {wid}[{sc.mode}] wp={sc.waypoint_idx} "
        ...
        if d < ARRIVE_RADIUS:
            ...
            print(f"[t_v6_2]  SCOUT {wid} [{sc.mode}] ARRIVED @{target}; "
                  f"next wp {new_idx} {wps[new_idx]}")
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
            except Exception as e:
                print(f"[t_v6_2]  scout move error: {e}")
            continue
        ...
        if stuck_reason is not None:
            print(f"[t_v6_2]  SCOUT {wid} [{sc.mode}] STUCK near "
            ...
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
                print(f"[t_v6_2]  SCOUT {wid} [{sc.mode}] -> wp {new_idx} "
                      f"{nxt} (blacklist size {len(sc.blacklist)})")
            except Exception as e:
                print(f"[t_v6_2]  scout skip-move error: {e}")
        else:
            ...
            if order_name_str not in ("Move", "MoveToAttack",
                ...
                try:
                    ...
                    print(f"[t_v6_2]  scout re-move error: {e}")
    ...
    for e in obs.get("enemies", []):
        if e.get("building") and e["unit_id"] not in known_enemies:
            ...
            print(f"[t_v6_2]  SCOUT SPOTTED enemy building "
    ...
    for n in obs.get("neutrals", []):
        ...
        if n["type"] in (176, 177, 178, 188):
            ...
            if dist_home > 1500:
                ...
                print(f"[t_v6_2]  SCOUT SPOTTED {kind} @({n['x']},{n['y']}) "
...
async def try_expand(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_2]  expand find_placement error: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        print(f"[t_v6_2]  EXPAND: no placement near cluster ({cx},{cy})")
        return None
    spot = spots[0]
    ...
    try:
        ...
        print(f"[t_v6_2]  expand build cmd error: {e}")
        return None

    pending_expansion_pts.add((cx, cy))
    print(f"[t_v6_2] FIRE  EXPAND CC @cluster ({cx},{cy}) "
...
async def try_build(c: Client, obs: dict, spec: BuildingSpec,
    ...
    try:
        ...
        print(f"[t_v6_2]  find_placement error {unit_type_name(spec.type_id)}: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        return None
    ...
    if spec.anchor != "geyser":
        ...
        for s in spots:
            ...
            if not in_mining_corridor(sx, sy, units, neutrals, main_type):
                keep.append(s)
        if not keep:
            print(f"[t_v6_2]  build {unit_type_name(spec.type_id)}: "
    ...
    try:
        ...
        print(f"[t_v6_2]  build cmd error {unit_type_name(spec.type_id)}: {e}")
        return None
    return Pending(
...
async def try_train_worker(c, obs, worker_type, main_type, cost_min):
    ...
    try:
        ...
        print(f"[t_v6_2]  train worker error: {e}")
        return None
...
async def try_train_unit(c, obs, spec: UnitSpec) -> Pending | None:
    ...
    try:
        ...
        print(f"[t_v6_2]  train unit error: {e}")
        return None
...
async def try_build_defense_structure(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_2]  defense find_placement error "
    ...
    try:
        ...
        print(f"[t_v6_2]  defense build cmd error "
              f"{unit_type_name(structure_type)}: {e}")
        return None
    return Pending(
...
async def phase_defense_line(c: Client, obs: dict,
                             ...
                             bunker_target: int,
                             turret_target: int,
                             opening_complete: bool) -> None:
    """v6_2 defense pass (extends v6_1):

    v6_2 addition: `opening_complete` gate. Until the caller
    reports the opening batch of Marines is out, this pass is a
    no-op. Prevents spending 100 min per bunker attempt (plus
    the SCV walk) before the base has anything to shoot with.
    ...
    if (bunker_c + bunker_ip < bunker_target
        ...
        if p is not None:
            ...
            print(f"[t_v6_2] FIRE  build:Bunker "
    ...
    if (eb_c > 0
        ...
        if p is not None:
            ...
            pending_workers.add(p.worker_id)
            print(f"[t_v6_2] FIRE  build:Missile_Turret "
    ...
    for tank in units:
        ...
        try:
            ...
            print(f"[t_v6_2]  DEFENSE move tank {tank['unit_id']} "
                  f"-> anchor ({anchor_x},{anchor_y}) "
                  f"was {d:.0f}px away")
        except Exception as e:
            print(f"[t_v6_2]  defense-tank move error: {e}")
    ...
    if bunkers:
        for m in units:
            ...
            try:
                ...
                print(f"[t_v6_2]  marine-ring move error: {e}")
...
async def try_upgrade(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_2] FIRE  {spec.kind}:{spec.label} @{src['unit_id']} "
              f"cost={spec.cost_min}/{spec.cost_gas}")
        return True
    except Exception as e:
        print(f"[t_v6_2]  {spec.kind} error: {e}")
        return False
...
async def run(c: Client, interval_sec: float,
    ...
    print(f"[t_v6_2] connected slot={c.welcome.slot} "
          f"frame={c.welcome.current_frame}")
    print(f"[t_v6_2] turtle config: "
    ...
    while True:
        ...
        if race is None:
            ...
            print(f"[t_v6_2] race={race} home=({home_x},{home_y}) "
                  f"map={map_w}x{map_h}")
            print(f"[t_v6_2] radial wps: {waypoints_by_mode['radial']}")
            print(f"[t_v6_2] zscan wps: {len(waypoints_by_mode['zscan'])} points "
        ...
        for spec in catalog_upgrades:
            ...
            if spec.kind == "upgrade":
                ...
                if lvl > 0:
                    ...
                    print(f"[t_v6_2] TOOK  upgrade:{spec.label} "
                          f"(level {lvl})")
            else:  # research
                if spec.enum_id in obs_tech:
                    completed_upgrades.add(key)
                    print(f"[t_v6_2] TOOK  research:{spec.label}")
        ...
        if allow_advance:
            ...
            print(f"[t_v6_2]  DEFENSE step advanced to {defense_step:.2f} "
            ...
            if push_mode:
                ...
                for tank in units:
                    ...
                    try:
                        ...
                        print(f"[t_v6_2]  PUSH unsiege tank "
                              f"{tank['unit_id']} to catch up to new anchor")
                    except Exception as e:
                        print(f"[t_v6_2]  push-unsiege error: {e}")
        ...
        for pu in units:
            ...
            if is_repairable(pu):
                damaged_mech += 1
        print(f"[t_v6_2] f={frame} min={r['minerals']} gas={r['gas']} "
                ...
                if cands:
                    ...
                    try:
                        ...
                        if spots:
                            ...
                            print(f"[t_v6_2] FIRE  build:Supply_Depot "
                                  f"({pyl_total2 + 1}/{pylon_target}) "
                                  f"anchor={anchor_pt}")
                        else:
                            budget["min"] += 100  # refund
                    except Exception as e:
                        print(f"[t_v6_2]  supply-depot fire error: {e}")
                        budget["min"] += 100
                else:
                    budget["min"] += 100
        ...
        if n_workers < worker_target and f"train:{worker_type}" not in pending:
            if reserve(worker_train_min, 0):
                ...
                if p is not None:
                    ...
                    print(f"[t_v6_2] FIRE  train:SCV ({n_workers + 1}/{worker_target})")
                else:
        ...
        if opening_complete and not getattr(run, "_opening_logged", False):
            print(f"[t_v6_2] OPENING COMPLETE @frame {frame}: "
        ...
        if (opening_complete
            ...
            if reserve(100, 0):
                ...
                if p is not None:
                    ...
                    pending_workers.add(p.worker_id)
                    print(f"[t_v6_2] FIRE  build:Refinery")
                else:
                    budget["min"] += 100
        ...
        for spec in catalog_buildings:
            ...
            if p is not None:
                pending[key] = p
                pending_workers.add(p.worker_id)
                print(f"[t_v6_2] FIRE  build:{unit_type_name(spec.type_id)}")
        ...
        for spec in catalog_units:
            ...
            if p is not None:
                pending[key] = p
                print(f"[t_v6_2] FIRE  train:{unit_type_name(spec.type_id)} "
            ...
            for tank in units:
                ...
                if should_siege:
                    try:
                        ...
                        print(f"[t_v6_2] FIRE  siege tank={tank['unit_id']} "
                              f"({reason})")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_2]  siege error: {e}")
                    ...
                    try:
                        await c.unsiege(unit_id=tank["unit_id"])
                        print(f"[t_v6_2] FIRE  unsiege tank={tank['unit_id']} "
                              f"(no enemy within {UNSIEGE_RANGE_PX}px, "
                              f"off anchor)")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_2]  unsiege error: {e}")
        ...
        if _TECH_SPIDER_MINES in obs_tech and tgt_x is not None:
            for vult in units:
                ...
                try:
                    ...
                    print(f"[t_v6_2] FIRE  place_mine vulture={vid} "
                          f"@({mx},{my}) drop {drop_idx+1}/3")
                    mine_fires += 1
                except Exception as e:
                    print(f"[t_v6_2]  place_mine error: {e}")
        ...
        if not move_done:
            ...
            if idle:
                ...
                try:
                    ...
                    print(f"[t_v6_2]  cover move error: {e}")
        if move_done and not stop_done:
            ...
            if cands:
                try:
                    ...
                    print(f"[t_v6_2]  cover stop error: {e}")
...
def entrypoint() -> None:
    p = argparse.ArgumentParser(
        prog="python3 -m python_agent.agents.t_agent_v6_2",
        description="Terran v6_2: v6_1 + army-first opening "
                    "(Barracks + Marines before Bunker / tech / "
                    "expansion).")
