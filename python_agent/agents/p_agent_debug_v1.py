"""p_agent_debug_v1: sync stress-test fork of p_agent_v4.

Same production/tech coverage as p_agent_v4, but with everything
that MOVES units removed. Combat units, workers, and scouts stay
where the sim spawned them. Buildings stay tight to the home
cluster (anchor is always nearest_own).

The point: on a two-observer soak, both observers should show
byte-identical scenes at every tick since the only wire content
is train/build/upgrade actions plus mining. Any visible or
hash-level mismatch between the observers is a real sync bug in
sync.h's action fan-out, not a consequence of "one observer
missed a Move and the units are now in different places."

Changes from p_agent_v4:
  - Priority 0.5 (phase_scout) DISABLED — no worker roaming.
  - Priority 8 (attack) DISABLED — combat units stand where they spawn.
  - Priority 9 (coverage move/stop) DISABLED — no filler movement.
  - _ANCHOR_STRATEGIES = ["nearest_own", ...] — buildings cluster
    around existing structures instead of drifting toward the map
    center.
  - `toward_center` fallback clamped hard (t=0..0.1, jitter ±48).
  - Default --interval-sec 0.1 (was 1.5) — faster action rate to
    reproduce burst-of-actions style sync bugs.
  - Default --base-target 2 (was 4) — smaller footprint.

Usage:
    python3 -m python_agent.agents.p_agent_debug_v1 <api_key>
"""
...
def race_catalogs(race: str):
    ...
    raise SystemExit(f"[p_dbg] race={race} not supported yet")
...
def verify_pending(pending: dict, obs: dict, stats: Stats,
    ...
    for key, p in pending.items():
        ...
        if cur_count > p.pre_count:
            ...
            print(f"[p_dbg] TOOK  {p.label():48s} "
        ...
        if age >= grace_frames:
            ...
            if n == 1 or n % 5 == 0:
                print(f"[p_dbg] REJECT {p.label():48s} after {age}f. n={n}. "
        ...
        if need > 0:
            ...
            for w in pool[:need]:
                ...
                try:
                    ...
                    print(f"[p_dbg]  gather-gas error: {e}")
    ...
    if mfs:
        for w in wu:
            ...
            try:
                ...
                print(f"[p_dbg]  gather-min error: {e}")
...
async def phase_scout(c: Client, obs: dict, worker_type: int,
    ...
    for wid in list(scouts.keys()):
        if wid not in live_ids:
            print(f"[p_dbg]  SCOUT worker {wid} died; unassigning")
            scouts.pop(wid, None)
    ...
    for mode, target_n in target_by_mode.items():
        ...
        for i in range(min(need, len(pool))):
            ...
            print(f"[p_dbg]  SCOUT worker {w['unit_id']} mode={mode} "
    ...
    def next_wp_idx(sc: Scout, n: int) -> int:
        ...
        if len(sc.blacklist) >= n:
            print(f"[p_dbg]  SCOUT {sc.worker_id} [{sc.mode}] all "
    ...
    for wid, sc in list(scouts.items()):
        ...
        if d < ARRIVE_RADIUS:
            ...
            print(f"[p_dbg]  SCOUT {wid} [{sc.mode}] ARRIVED @{target}; "
                  f"next wp {new_idx} {wps[new_idx]}")
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
            except Exception as e:
                print(f"[p_dbg]  scout move error: {e}")
            continue
        ...
        if stuck_reason is not None:
            print(f"[p_dbg]  SCOUT {wid} [{sc.mode}] STUCK near "
            ...
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
                print(f"[p_dbg]  SCOUT {wid} [{sc.mode}] -> wp {new_idx} "
                      f"{nxt} (blacklist size {len(sc.blacklist)})")
            except Exception as e:
                print(f"[p_dbg]  scout skip-move error: {e}")
        else:
            ...
            if order_name_str not in ("Move", "MoveToAttack",
                ...
                try:
                    ...
                    print(f"[p_dbg]  scout re-move error: {e}")
    ...
    for e in obs.get("enemies", []):
        if e.get("building") and e["unit_id"] not in known_enemies:
            ...
            print(f"[p_dbg]  SCOUT SPOTTED enemy building "
    ...
    for n in obs.get("neutrals", []):
        ...
        if n["type"] in (176, 177, 178, 188):
            ...
            if dist_home > 1500:
                ...
                print(f"[p_dbg]  SCOUT SPOTTED {kind} @({n['x']},{n['y']}) "
...
async def try_expand(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[p_dbg]  expand find_placement error: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        print(f"[p_dbg]  EXPAND: no placement near cluster ({cx},{cy})")
        return None
    spot = spots[0]
    ...
    try:
        ...
        print(f"[p_dbg]  expand build cmd error: {e}")
        return None

    pending_expansion_pts.add((cx, cy))
    print(f"[p_dbg] FIRE  EXPAND Nexus @cluster ({cx},{cy}) "
...
async def try_build(c: Client, obs: dict, spec: BuildingSpec,
    ...
    try:
        ...
        print(f"[p_dbg]  find_placement error {unit_type_name(spec.type_id)}: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        return None
    ...
    try:
        ...
        print(f"[p_dbg]  build cmd error {unit_type_name(spec.type_id)}: {e}")
        return None
    return Pending(
...
async def try_train_worker(c, obs, worker_type, main_type, cost_min):
    ...
    try:
        ...
        print(f"[p_dbg]  train worker error: {e}")
        return None
...
async def try_train_unit(c, obs, spec: UnitSpec) -> Pending | None:
    ...
    try:
        ...
        print(f"[p_dbg]  train unit error: {e}")
        return None
...
async def try_upgrade(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[p_dbg] FIRE  {spec.kind}:{spec.label} @{src['unit_id']} "
              f"cost={spec.cost_min}/{spec.cost_gas}")
        return True
    except Exception as e:
        print(f"[p_dbg]  {spec.kind} error: {e}")
        return False
...
async def run(c: Client, interval_sec: float,
    ...
    print(f"[p_dbg] connected slot={c.welcome.slot} "
    ...
    while True:
        ...
        if race is None:
            ...
            print(f"[p_dbg] race={race} home=({home_x},{home_y}) "
                  f"map={map_w}x{map_h}")
            print(f"[p_dbg] radial wps: {waypoints_by_mode['radial']}")
            print(f"[p_dbg] zscan wps: {len(waypoints_by_mode['zscan'])} points "
        ...
        for spec in catalog_upgrades:
            ...
            if spec.kind == "upgrade":
                ...
                if lvl > 0:
                    ...
                    print(f"[p_dbg] TOOK  upgrade:{spec.label} "
                          f"(level {lvl})")
            else:  # research
                if spec.enum_id in obs_tech:
                    completed_upgrades.add(key)
                    print(f"[p_dbg] TOOK  research:{spec.label}")
        ...
        print(f"[p_dbg] f={frame} min={r['minerals']} gas={r['gas']} "
                ...
                if cands:
                    ...
                    try:
                        ...
                        if spots:
                            ...
                            print(f"[p_dbg] FIRE  build:Pylon "
                                  f"({pyl_total2 + 1}/{pylon_target}) "
                                  f"anchor={anchor_pt}")
                        else:
                            budget["min"] += 100  # refund
                    except Exception as e:
                        print(f"[p_dbg]  pylon fire error: {e}")
                        budget["min"] += 100
                else:
                    budget["min"] += 100
        ...
        if n_workers < worker_target and f"train:{worker_type}" not in pending:
            if reserve(worker_train_min, 0):
                ...
                if p is not None:
                    ...
                    print(f"[p_dbg] FIRE  train:Probe ({n_workers + 1}/{worker_target})")
                else:
        ...
        if gas_c + gas_ip == 0 and f"build:{gas_bld}" not in pending:
            if reserve(100, 0):
                ...
                if p is not None:
                    ...
                    print(f"[p_dbg] FIRE  build:Assimilator")
                else:
                    budget["min"] += 100
        ...
        for spec in catalog_buildings:
            ...
            if p is not None:
                pending[key] = p
                pending_workers.add(p.worker_id)
                print(f"[p_dbg] FIRE  build:{unit_type_name(spec.type_id)}")
        ...
        for spec in catalog_units:
            ...
            if p is not None:
                pending[key] = p
                print(f"[p_dbg] FIRE  train:{unit_type_name(spec.type_id)} "
        ...
        for pu in units:
            ...
            try:
                ...
                print(f"[p_dbg] FIRE  train_fighter "
                      f"{unit_type_name(pu['type'])} {pu['unit_id']} "
                      f"({fc}+{fq}/{fm})")
                fighter_fires += 1
            except Exception as e:
                print(f"[p_dbg]  train_fighter error: {e}")
                budget["min"] += cost

        # Priority 8 (attack) and Priority 9 (coverage verbs) DISABLED
        # for p_debug. This agent must never issue move/attack orders
        # so combat units accumulate at their spawn points, making the
        # observer state hash easy to diff visually across observers.
...
def entrypoint() -> None:
    p = argparse.ArgumentParser(
        prog="python3 -m python_agent.agents.p_agent_debug_v1",
