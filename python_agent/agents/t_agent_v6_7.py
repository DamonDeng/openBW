"""t_agent_v6_7: v6_6 + line-gap turret build fix.

v6_6's tank line pushed all the way to 0.85 of the way toward the
enemy base, and the turret-chain logic correctly wanted to place a
Missile Turret at each historical anchor. But the SCV picker chose
the SCV nearest the TARGET (front-line coordinates), which meant an
SCV already in contested ground got picked, walked further into
enemy territory, and died to cloaked units before finishing the
build. Result: 20+ REJECTs, no forward-line turrets, tanks with no
detection.

Changes vs. v6_6:
  * SCV picker: try_build_defense_structure now picks the SCV
    nearest to the OWN CC closest to the target -- not the SCV
    nearest to the target itself. Line-gap turrets get an SCV
    from the safest own base with a clean walk-out.
  * Enemy-proximity veto: in phase_defense_line pass 2b (line-
    history turrets), skip candidate positions with a visible
    enemy within TURRET_BUILD_ENEMY_VETO_PX (500 px). Tanks
    clear the ground first, then the turret builds.

Everything else is v6_6 verbatim.

--- inherited notes from v6_6: ---
...
def race_catalogs(race: str):
    ...
    raise SystemExit(f"[t_v6_7] race={race} not supported (this is the "
...
def verify_pending(pending: dict, obs: dict, stats: Stats,
    ...
    for key, p in pending.items():
        ...
        if cur_count > p.pre_count:
            ...
            print(f"[t_v6_7] TOOK  {p.label():48s} "
        ...
        if age >= grace_frames:
            ...
            if n == 1 or n % 5 == 0:
                print(f"[t_v6_7] REJECT {p.label():48s} after {age}f. n={n}. "
        ...
        if need > 0:
            ...
            for w in pool[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  gather-gas error: {e}")
    ...
    if mfs:
        for w in wu:
            ...
            try:
                ...
                print(f"[t_v6_7]  gather-min error: {e}")
...
async def phase_scout(c: Client, obs: dict, worker_type: int,
    ...
    for wid in list(scouts.keys()):
        if wid not in live_ids:
            print(f"[t_v6_7]  SCOUT worker {wid} died; unassigning")
            scouts.pop(wid, None)
    ...
    for mode, target_n in target_by_mode.items():
        ...
        for i in range(min(need, len(pool))):
            ...
            print(f"[t_v6_7]  SCOUT worker {w['unit_id']} mode={mode} "
    ...
    def next_wp_idx(sc: Scout, n: int) -> int:
        ...
        if len(sc.blacklist) >= n:
            print(f"[t_v6_7]  SCOUT {sc.worker_id} [{sc.mode}] all "
    ...
    for wid, sc in list(scouts.items()):
        ...
        print(f"[t_v6_7/SCOUT] {wid}[{sc.mode}] wp={sc.waypoint_idx} "
        ...
        if d < ARRIVE_RADIUS:
            ...
            print(f"[t_v6_7]  SCOUT {wid} [{sc.mode}] ARRIVED @{target}; "
                  f"next wp {new_idx} {wps[new_idx]}")
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
            except Exception as e:
                print(f"[t_v6_7]  scout move error: {e}")
            continue
        ...
        if stuck_reason is not None:
            print(f"[t_v6_7]  SCOUT {wid} [{sc.mode}] STUCK near "
            ...
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
                print(f"[t_v6_7]  SCOUT {wid} [{sc.mode}] -> wp {new_idx} "
                      f"{nxt} (blacklist size {len(sc.blacklist)})")
            except Exception as e:
                print(f"[t_v6_7]  scout skip-move error: {e}")
        else:
            ...
            if order_name_str not in ("Move", "MoveToAttack",
                ...
                try:
                    ...
                    print(f"[t_v6_7]  scout re-move error: {e}")
    ...
    for e in obs.get("enemies", []):
        if e.get("building") and e["unit_id"] not in known_enemies:
            ...
            print(f"[t_v6_7]  SCOUT SPOTTED enemy building "
    ...
    for n in obs.get("neutrals", []):
        ...
        if n["type"] in (176, 177, 178, 188):
            ...
            if dist_home > 1500:
                ...
                print(f"[t_v6_7]  SCOUT SPOTTED {kind} @({n['x']},{n['y']}) "
...
async def try_expand(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_7]  expand find_placement error: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        print(f"[t_v6_7]  EXPAND: no placement near cluster ({cx},{cy})")
        return None
    spot = spots[0]
    ...
    try:
        ...
        print(f"[t_v6_7]  expand build cmd error: {e}")
        return None

    pending_expansion_pts.add((cx, cy))
    print(f"[t_v6_7] FIRE  EXPAND CC @cluster ({cx},{cy}) "
...
async def try_build(c: Client, obs: dict, spec: BuildingSpec,
    ...
    try:
        ...
        print(f"[t_v6_7]  find_placement error {unit_type_name(spec.type_id)}: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        return None
    ...
    if spec.anchor != "geyser":
        ...
        for s in spots:
            ...
            if not in_mining_corridor(sx, sy, units, neutrals, main_type):
                keep.append(s)
        if not keep:
            print(f"[t_v6_7]  build {unit_type_name(spec.type_id)}: "
    ...
    try:
        ...
        print(f"[t_v6_7]  build cmd error {unit_type_name(spec.type_id)}: {e}")
        return None
    return Pending(
...
async def try_train_worker(c, obs, worker_type, main_type, cost_min,
    ...
    try:
        ...
        print(f"[t_v6_7]  train worker error: {e}")
        return None
...
async def try_train_unit(c, obs, spec: UnitSpec) -> Pending | None:
    ...
    try:
        ...
        print(f"[t_v6_7]  train unit error: {e}")
        return None
...
# v6_7: when the line-gap turret pass considers a candidate
# position, skip it if there's any visible enemy within this
# radius. Prevents the "SCV walks toward the target, dies to
# a cloaked unit" cascade the v6_6 smoke test showed.
# 500 px = ~15 tiles; larger than a marine's sight (7 tiles) so
# even a partial view of the target area is enough to hold off.
TURRET_BUILD_ENEMY_VETO_PX = 500
...
async def try_build_defense_structure(c: Client, obs: dict,
    ...
    """Place a Bunker / Missile Turret at (anchor_x, anchor_y).

    v6_7 SCV picker: pick the SCV nearest to the OWN CC closest to
    the target. Previously we picked the SCV nearest to the target
    itself, which for a forward line-gap turret meant picking an
    SCV already in contested ground -- the SCV then walked
    further into enemy territory and died. Picking from the
    safest CC gives the SCV a clean walk-out from friendly ground.
    """
    r = obs["resources"]
    ...
    # Find own CC closest to the target, use it as the SCV pool.
    own_ccs = [u for u in units if u["type"] == main_type
               and u.get("completed") and not u.get("flying")]
    if own_ccs:
        cc = min(own_ccs, key=lambda u: dist_pixels(
            u["x"], u["y"], anchor_x, anchor_y))
        cc_x, cc_y = cc["x"], cc["y"]
        worker = min(cands, key=lambda u: dist_pixels(
            u["x"], u["y"], cc_x, cc_y))
    else:
        # Degenerate: no completed CCs -- fall back to old picker.
        worker = min(cands, key=lambda u: dist_pixels(
            u["x"], u["y"], anchor_x, anchor_y))
    ...
    try:
        ...
        print(f"[t_v6_7]  defense find_placement error "
    ...
    try:
        ...
        print(f"[t_v6_7]  defense build cmd error "
              f"{unit_type_name(structure_type)}: {e}")
        return None
    return Pending(
...
async def phase_defense_line(c: Client, obs: dict,
    ...
    if (bunker_c + bunker_ip < bunker_target
        ...
        if p is not None:
            ...
            print(f"[t_v6_7] FIRE  build:Bunker "
    ...
    if (eb_c > 0
        ...
        if p is not None:
            ...
            pending_workers.add(p.worker_id)
            print(f"[t_v6_7] FIRE  build:Missile_Turret "
    ...
    # DEFENSE_TURRET_COVER_PX of it, queue one. Only fire one
    # gap turret per tick (piggybacks on base turret pass's
    # 1-per-tick budget) so we don't burn 4x75 min on one call.
    #
    # v6_7 addition: skip candidate positions with a visible enemy
    # within TURRET_BUILD_ENEMY_VETO_PX. The SCV would either die
    # on the walk-out or the build spot itself would be under
    # attack. Tanks will clear the ground first; the turret can
    # then build safely on a subsequent tick.
    if (eb_c > 0
        ...
        enemies_visible_here = obs.get("enemies", [])
        ...
        for hx, hy in defense_line_history:
            ...
            # v6_7 enemy veto.
            enemy_near = False
            for e in enemies_visible_here:
                if dist_pixels(e["x"], e["y"], hx, hy) <= TURRET_BUILD_ENEMY_VETO_PX:
                    enemy_near = True
                    break
            if enemy_near:
                if frame % 500 == 0:
                    print(f"[t_v6_7]  turret line_gap=({hx},{hy}) "
                          f"skipped: enemy within "
                          f"{TURRET_BUILD_ENEMY_VETO_PX}px")
                continue
            gap_pt = (hx, hy)
            break
    ...
    for tank in units:
        ...
        try:
            ...
            print(f"[t_v6_7]  tank-arc move error: {e}")
    ...
    if bunkers:
        for m in units:
            ...
            try:
                ...
                print(f"[t_v6_7]  marine-ring move error: {e}")
    ...
    for cc in expansions:
        ...
        if g["bunker"] == -1:
            ...
            if near_bunkers:
                ...
                print(f"[t_v6_7]  GARRISON cc{cc_id}: adopted "
                      f"bunker {g['bunker']}")
        ...
        if len(g["marines"]) < 2:
            ...
            for m in avail_marines[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  GARRISON cc{cc_id}: assign "
                          f"marine {m['unit_id']} -> ({dst_x},{dst_y})")
                except Exception as e:
                    print(f"[t_v6_7]  garrison-marine move error: {e}")
        ...
        if g["tank"] == -1:
            ...
            if avail_tanks:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  garrison-tank move error: {e}")
...
async def try_upgrade(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6_7] FIRE  {spec.kind}:{spec.label} @{src['unit_id']} "
              f"cost={spec.cost_min}/{spec.cost_gas}")
        return True
    except Exception as e:
        print(f"[t_v6_7]  {spec.kind} error: {e}")
        return False
...
async def run(c: Client, interval_sec: float,
    ...
    print(f"[t_v6_7] connected slot={c.welcome.slot} "
          f"frame={c.welcome.current_frame}")
    print(f"[t_v6_7] turtle config: "
    ...
    while True:
        ...
        if race is None:
            ...
            print(f"[t_v6_7] race={race} home=({home_x},{home_y}) "
                  f"map={map_w}x{map_h}")
            print(f"[t_v6_7] radial wps: {waypoints_by_mode['radial']}")
            print(f"[t_v6_7] zscan wps: {len(waypoints_by_mode['zscan'])} points "
            ...
            if exp_key in pending_expansion_cluster and exp_key not in pending:
                ...
                if nx_now <= len(own_of_type(units, main_type, only_complete=False)) - 1:
                    ...
                    if n >= EXPANSION_REJECT_THRESHOLD and cluster_pt not in cluster_blacklist:
                        ...
                        print(f"[t_v6_7]  EXPAND BLACKLIST cluster "
                              f"{cluster_pt}: {n} REJECTs in a row. "
                              f"Won't try again this game.")
                    else:
                        print(f"[t_v6_7]  EXPAND REJECT tracked for "
        ...
        for spec in catalog_upgrades:
            ...
            if spec.kind == "upgrade":
                ...
                if lvl > 0:
                    ...
                    print(f"[t_v6_7] TOOK  upgrade:{spec.label} "
                          f"(level {lvl})")
            else:  # research
                if spec.enum_id in obs_tech:
                    completed_upgrades.add(key)
                    print(f"[t_v6_7] TOOK  research:{spec.label}")
        ...
        if allow_advance:
            ...
            print(f"[t_v6_7]  DEFENSE step advanced to {defense_step:.2f} "
            ...
            for tank in units:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  rotate-unsiege error: {e}")
            if released > 0:
                print(f"[t_v6_7]  ROTATE unsiege parity={tank_slot_next_release}: "
        ...
        for pu in units:
            ...
            if is_repairable(pu):
                damaged_mech += 1
        print(f"[t_v6_7] f={frame} min={r['minerals']} gas={r['gas']} "
                ...
                if cands:
                    ...
                    try:
                        ...
                        if spots:
                            ...
                            print(f"[t_v6_7] FIRE  build:Supply_Depot "
                                  f"({pyl_total2 + 1}/{pylon_target}) "
                                  f"anchor={anchor_pt}")
                        else:
                            budget["min"] += 100  # refund
                    except Exception as e:
                        print(f"[t_v6_7]  supply-depot fire error: {e}")
                        budget["min"] += 100
                else:
                    budget["min"] += 100
        ...
        if n_workers < worker_target:
            ...
            for cc in completed_ccs:
                ...
                if not reserve(worker_train_min, 0):
                    if frame % 500 == 0:
                        print(f"[t_v6_7]  WORKER-STALL: want SCV "
                ...
                if result is not None:
                    ...
                    print(f"[t_v6_7] FIRE  train:SCV ({n_workers + 1}/{worker_target}) "
                          f"@cc{cc_id}")
                else:
        ...
        if opening_complete and not getattr(run, "_opening_logged", False):
            print(f"[t_v6_7] OPENING COMPLETE @frame {frame}: "
        ...
        if (opening_complete
            ...
            if reserve(100, 0):
                ...
                if p is not None:
                    ...
                    pending_workers.add(p.worker_id)
                    print(f"[t_v6_7] FIRE  build:Refinery")
                else:
                    budget["min"] += 100
        ...
        if depleted_bases > 0 and frame % 500 == 0:
            print(f"[t_v6_7]  DEPLETION detected: {depleted_bases} base(s) "
        ...
        for spec in catalog_buildings:
            ...
            if p is not None:
                pending[key] = p
                pending_workers.add(p.worker_id)
                print(f"[t_v6_7] FIRE  build:{unit_type_name(spec.type_id)}")
        ...
        for parent_type, addon_type, amin, agas, aname in _ADDON_ATTACHMENTS:
            ...
            for parent in own_of_type(units, parent_type):
                ...
                try:
                    ...
                    print(f"[t_v6_7] FIRE  addon:{aname} on "
                    ...
                    print(f"[t_v6_7]  addon fire error: {e}")
        ...
        for spec in catalog_units:
            ...
            if p is not None:
                pending[key] = p
                print(f"[t_v6_7] FIRE  train:{unit_type_name(spec.type_id)} "
        ...
        for bld in units:
            ...
            if bld.get("flying") and bld["unit_id"] in addon_lifted:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  land find_placement error: {e}")
                    continue
            ...
            try:
                ...
                print(f"[t_v6_7] FIRE  lift Terran_Factory "
                      f"{bld['unit_id']} for addon relocation "
                      f"(retries exhausted)")
                lifted = True
            except Exception as e:
                print(f"[t_v6_7]  addon-lift error: {e}")
            ...
            for tank in units:
                ...
                if is_stale:
                    try:
                        ...
                        if slot is not None:
                            ...
                            slot_dist = -1
                        print(f"[t_v6_7] FIRE  stale-unsiege "
                              f"tank={tank['unit_id']} "
                        ...
                        print(f"[t_v6_7]  stale-unsiege error: {e}")
                ...
                if should_siege:
                    try:
                        ...
                        print(f"[t_v6_7] FIRE  siege tank={tank['unit_id']} "
                              f"({reason})")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_7]  siege error: {e}")
                    ...
                    try:
                        await c.unsiege(unit_id=tank["unit_id"])
                        print(f"[t_v6_7] FIRE  unsiege tank={tank['unit_id']} "
                              f"(no enemy within {UNSIEGE_RANGE_PX}px, "
                              f"off slot)")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6_7]  unsiege error: {e}")
        ...
        if _TECH_SPIDER_MINES in obs_tech and tgt_x is not None:
            for vult in units:
                ...
                try:
                    ...
                    print(f"[t_v6_7] FIRE  place_mine vulture={vid} "
                          f"@({mx},{my}) drop {drop_idx+1}/3")
                    mine_fires += 1
                except Exception as e:
                    print(f"[t_v6_7]  place_mine error: {e}")
        ...
        if not move_done:
            ...
            if idle:
                ...
                try:
                    ...
                    print(f"[t_v6_7]  cover move error: {e}")
        if move_done and not stop_done:
            ...
            if cands:
                try:
                    ...
                    print(f"[t_v6_7]  cover stop error: {e}")
...
def entrypoint() -> None:
    p = argparse.ArgumentParser(
        prog="python3 -m python_agent.agents.t_agent_v6_7",
        description="Terran v6_7: v6_6 + safer SCV picker for "
                    "forward turret builds (pick from nearest own "
                    "CC, not from the target) + enemy-proximity "
                    "veto so SCVs stop walking into cloak "
                    "territory.")
