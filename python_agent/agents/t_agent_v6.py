"""t_agent_v6: v5 + turtle defense doctrine.

Same passes as v5 (SCV repair, addons, siege, mine drop, lift-to-
safety) but the strategic tempo changes: hold ground first, expand
second. Building placement is compact and defense-aware.
...
def race_catalogs(race: str):
    ...
    raise SystemExit(f"[t_v6] race={race} not supported (this is the "
...
def verify_pending(pending: dict, obs: dict, stats: Stats,
    ...
    for key, p in pending.items():
        ...
        if age >= grace_frames:
            ...
            if n == 1 or n % 5 == 0:
                print(f"[t_v6] REJECT {p.label():48s} after {age}f. n={n}. "
        ...
        if need > 0:
            ...
            for w in pool[:need]:
                ...
                try:
                    ...
                    print(f"[t_v6]  gather-gas error: {e}")
    ...
    if mfs:
        for w in wu:
            ...
            try:
                ...
                print(f"[t_v6]  gather-min error: {e}")
...
async def phase_scout(c: Client, obs: dict, worker_type: int,
    ...
    for wid in list(scouts.keys()):
        if wid not in live_ids:
            print(f"[t_v6]  SCOUT worker {wid} died; unassigning")
            scouts.pop(wid, None)
    ...
    for mode, target_n in target_by_mode.items():
        ...
        for i in range(min(need, len(pool))):
            ...
            print(f"[t_v6]  SCOUT worker {w['unit_id']} mode={mode} "
    ...
    def next_wp_idx(sc: Scout, n: int) -> int:
        ...
        if len(sc.blacklist) >= n:
            print(f"[t_v6]  SCOUT {sc.worker_id} [{sc.mode}] all "
    ...
    for wid, sc in list(scouts.items()):
        ...
        print(f"[t_v6/SCOUT] {wid}[{sc.mode}] wp={sc.waypoint_idx} "
        ...
        if d < ARRIVE_RADIUS:
            ...
            print(f"[t_v6]  SCOUT {wid} [{sc.mode}] ARRIVED @{target}; "
                  f"next wp {new_idx} {wps[new_idx]}")
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
            except Exception as e:
                print(f"[t_v6]  scout move error: {e}")
            continue
        ...
        if stuck_reason is not None:
            print(f"[t_v6]  SCOUT {wid} [{sc.mode}] STUCK near "
            ...
            try:
                nxt = wps[new_idx]
                await c.move(unit_id=wid, x=nxt[0], y=nxt[1])
                print(f"[t_v6]  SCOUT {wid} [{sc.mode}] -> wp {new_idx} "
                      f"{nxt} (blacklist size {len(sc.blacklist)})")
            except Exception as e:
                print(f"[t_v6]  scout skip-move error: {e}")
        else:
            ...
            if order_name_str not in ("Move", "MoveToAttack",
                ...
                try:
                    ...
                    print(f"[t_v6]  scout re-move error: {e}")
    ...
    for e in obs.get("enemies", []):
        if e.get("building") and e["unit_id"] not in known_enemies:
            ...
            print(f"[t_v6]  SCOUT SPOTTED enemy building "
    ...
    for n in obs.get("neutrals", []):
        ...
        if n["type"] in (176, 177, 178, 188):
            ...
            if dist_home > 1500:
                ...
                print(f"[t_v6]  SCOUT SPOTTED {kind} @({n['x']},{n['y']}) "
...
async def try_expand(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6]  expand find_placement error: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        print(f"[t_v6]  EXPAND: no placement near cluster ({cx},{cy})")
        return None
    spot = spots[0]
    ...
    try:
        ...
        print(f"[t_v6]  expand build cmd error: {e}")
        return None

    pending_expansion_pts.add((cx, cy))
    print(f"[t_v6] FIRE  EXPAND CC @cluster ({cx},{cy}) "
...
async def try_build(c: Client, obs: dict, spec: BuildingSpec,
    ...
    try:
        ...
        print(f"[t_v6]  find_placement error {unit_type_name(spec.type_id)}: {e}")
        return None
    spots = resp.get("spots", [])
    if not spots:
        return None
    ...
    if spec.anchor != "geyser":
        ...
        for s in spots:
            ...
            if not in_mining_corridor(sx, sy, units, neutrals, main_type):
                keep.append(s)
        if not keep:
            print(f"[t_v6]  build {unit_type_name(spec.type_id)}: "
    ...
    try:
        ...
        print(f"[t_v6]  build cmd error {unit_type_name(spec.type_id)}: {e}")
        return None
    return Pending(
...
async def try_train_worker(c, obs, worker_type, main_type, cost_min):
    ...
    try:
        ...
        print(f"[t_v6]  train worker error: {e}")
        return None
...
async def try_train_unit(c, obs, spec: UnitSpec) -> Pending | None:
    ...
    try:
        ...
        print(f"[t_v6]  train unit error: {e}")
        return None
...
# 0.20 = first line, 20 % of the way toward known enemy base
# 0.30 = second, 0.40 = third, capped at DEFENSE_STEP_MAX
DEFENSE_STEP_START = 0.20
DEFENSE_STEP_DELTA = 0.10
DEFENSE_STEP_MAX = 0.60
...
# "Enemy nearby" radius used to VETO a step-forward.
...
async def try_build_defense_structure(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6]  defense find_placement error "
    ...
    try:
        ...
        print(f"[t_v6]  defense build cmd error "
              f"{unit_type_name(structure_type)}: {e}")
        return None
    return Pending(
...
async def phase_defense_line(c: Client, obs: dict,
                             ...
                             pending: dict,
                             reserve,
                             bunker_target: int) -> None:
    """v6 defense pass.

    (1) Ensure `bunker_target` bunkers exist near the current
        defense anchor (Terran_Bunker). Skips if a bunker build
        is already in flight.
    (2) Ensure at least 1 Missile Turret near the anchor (spawns
        detection + anti-air; also cheap AoE damage sink).
    (3) Move any completed Siege Tank (both modes) that's more
        than DEFENSE_LINE_TIGHT_PX from the anchor toward the
        anchor. Sieging happens in Priority 7.7 -- this pass
        only moves them into position.
    (4) Marines: move any idle Marine to a random point in a ring
        around the nearest bunker. `load` verb is not exposed by
        the server yet (see SERVER_ASKS.md); in-ring marines will
        still shoot enemies passing by.
    """
    units = obs["units"]
    frame = obs["current_frame"]
    ...
    if (bunker_c + bunker_ip < bunker_target
        ...
        if p is not None:
            ...
            print(f"[t_v6] FIRE  build:Bunker "
                  f"({bunker_c + bunker_ip + 1}/{bunker_target}) "
                  f"@anchor=({anchor_x},{anchor_y})")
        else:
            # refund via the caller's budget dict handled by reserve
            # return-path -- but reserve() doesn't expose refund.
            # The re-fire cost next tick is the same 100 min so
            # this is a wash; log for visibility.
            pass

    # (2) One Missile Turret near the anchor. Needs Engineering Bay
    # completed for the sim to accept placement -- if we don't have
    # one yet, sim silent-rejects and the pending stays until the
    # grace_frames verify drops it. Cost: 75 min.
    ...
    if (eb_c > 0
        ...
        if p is not None:
            ...
            pending_workers.add(p.worker_id)
            print(f"[t_v6] FIRE  build:Missile_Turret "
                  f"@anchor=({anchor_x},{anchor_y})")
    ...
    for tank in units:
        ...
        try:
            ...
            print(f"[t_v6]  DEFENSE move tank {tank['unit_id']} "
                  f"-> anchor ({anchor_x},{anchor_y}) "
                  f"was {d:.0f}px away")
        except Exception as e:
            print(f"[t_v6]  defense-tank move error: {e}")
    ...
    if bunkers:
        for m in units:
            ...
            try:
                ...
                print(f"[t_v6]  marine-ring move error: {e}")
...
async def try_upgrade(c: Client, obs: dict,
    ...
    try:
        ...
        print(f"[t_v6] FIRE  {spec.kind}:{spec.label} @{src['unit_id']} "
              f"cost={spec.cost_min}/{spec.cost_gas}")
        return True
    except Exception as e:
        print(f"[t_v6]  {spec.kind} error: {e}")
        return False
...
async def run(c: Client, interval_sec: float,
              ...
              base_target: int,
              bunker_target: int,
              defense_only: bool) -> None:
    print(f"[t_v6] connected slot={c.welcome.slot} "
          f"frame={c.welcome.current_frame}")
    print(f"[t_v6] turtle config: bunker_target={bunker_target} "
          f"defense_only={defense_only} base_target={base_target}")
    ...
    while True:
        ...
        if race is None:
            ...
            print(f"[t_v6] race={race} home=({home_x},{home_y}) "
                  f"map={map_w}x{map_h}")
            print(f"[t_v6] radial wps: {waypoints_by_mode['radial']}")
            print(f"[t_v6] zscan wps: {len(waypoints_by_mode['zscan'])} points "
        ...
        for spec in catalog_upgrades:
            ...
            if spec.kind == "upgrade":
                ...
                if lvl > 0:
                    ...
                    print(f"[t_v6] TOOK  upgrade:{spec.label} "
                          f"(level {lvl})")
            else:  # research
                if spec.enum_id in obs_tech:
                    completed_upgrades.add(key)
                    print(f"[t_v6] TOOK  research:{spec.label}")
        ...
        # v6: defense anchor = point at `defense_step` along home -> tgt.
        # We advance the step only when the line is tight (all tanks
        # within DEFENSE_LINE_TIGHT_PX of the current anchor) AND no
        # enemy has been seen within DEFENSE_ENEMY_VETO_PX in the last
        # tick's visible enemies. Rate-limit: at most one advance per
        # 500 frames so we can't step-forward faster than the tanks
        # can catch up between ticks.
        ...
        for e in obs.get("enemies", []):
            if dist_pixels(e["x"], e["y"],
                ...
                break
        if (defense_step < DEFENSE_STEP_MAX
                and defense_line_ready(units, anchor_x, anchor_y)
                and not enemy_near_anchor
                and frame - last_step_advance_frame > 500):
            defense_step = min(DEFENSE_STEP_MAX,
                               defense_step + DEFENSE_STEP_DELTA)
            last_step_advance_frame = frame
            print(f"[t_v6]  DEFENSE step advanced to {defense_step:.2f} "
                  f"@frame {frame}; new anchor "
                  f"=({defense_anchor(home_x, home_y, tgt_x, tgt_y, defense_step, map_w, map_h)})")
        ...
        for pu in units:
            ...
            if is_repairable(pu):
                damaged_mech += 1
        print(f"[t_v6] f={frame} min={r['minerals']} gas={r['gas']} "
                ...
                if cands:
                    ...
                    try:
                        ...
                        if spots:
                            ...
                            print(f"[t_v6] FIRE  build:Supply_Depot "
                                  f"({pyl_total2 + 1}/{pylon_target}) "
                                  f"anchor={anchor_pt}")
                        else:
                            budget["min"] += 100  # refund
                    except Exception as e:
                        print(f"[t_v6]  supply-depot fire error: {e}")
                        budget["min"] += 100
                else:
                    budget["min"] += 100
        ...
        if n_workers < worker_target and f"train:{worker_type}" not in pending:
            if reserve(worker_train_min, 0):
                ...
                if p is not None:
                    ...
                    print(f"[t_v6] FIRE  train:SCV ({n_workers + 1}/{worker_target})")
                else:
        ...
        if gas_c + gas_ip == 0 and f"build:{gas_bld}" not in pending:
            if reserve(100, 0):
                ...
                if p is not None:
                    ...
                    pending_workers.add(p.worker_id)
                    print(f"[t_v6] FIRE  build:Refinery")
                else:
                    budget["min"] += 100

        # ---- v6 Priority 4.25: defense line.
        # Bunkers + Missile Turret + tank-to-anchor. This runs BEFORE
        # expansion and catalog buildings so it gets first crack at
        # the mineral budget -- defensive structures are cheaper than
        # a CC and much more valuable early game.
        await phase_defense_line(
            c, obs, anchor_x, anchor_y,
            worker_type, main_type, pending_workers, pending,
            reserve, bunker_target)
        ...
        # v6 gating: don't expand until the turtle line is up.
        _BUNKER_TYPE = UNIT_TYPES_BY_NAME["Terran_Bunker"]
        _TANK_MODE_TYPE = UNIT_TYPES_BY_NAME["Terran_Siege_Tank_Tank_Mode"]
        _SIEGE_MODE_TYPE = UNIT_TYPES_BY_NAME["Terran_Siege_Tank_Siege_Mode"]
        bunker_ready, _ = count_units(units, _BUNKER_TYPE)
        tank_tm, _ = count_units(units, _TANK_MODE_TYPE)
        tank_sm, _ = count_units(units, _SIEGE_MODE_TYPE)
        turtle_ready = (bunker_ready >= 2
                        and (tank_tm + tank_sm) >= 3)
        ...
        if (nx_c + nx_ip < base_target and nx_key not in pending
            ...
            # One-line log so it's visible in the tick output that
            # we're deliberately not expanding.
            if frame % 200 == 0:
                print(f"[t_v6]  EXPAND holdoff: bunker_ready="
                      f"{bunker_ready}/2 tanks={tank_tm + tank_sm}/3")
        ...
        for spec in catalog_buildings:
            ...
            if p is not None:
                pending[key] = p
                pending_workers.add(p.worker_id)
                print(f"[t_v6] FIRE  build:{unit_type_name(spec.type_id)}")
        ...
        for spec in catalog_units:
            ...
            if p is not None:
                pending[key] = p
                print(f"[t_v6] FIRE  train:{unit_type_name(spec.type_id)} "
            ...
            for tank in units:
                ...
                if should_siege:
                    try:
                        ...
                        print(f"[t_v6] FIRE  siege tank={tank['unit_id']} "
                              f"({reason})")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6]  siege error: {e}")
                    ...
                    try:
                        ...
                        print(f"[t_v6] FIRE  unsiege tank={tank['unit_id']} "
                              f"(no enemy within {UNSIEGE_RANGE_PX}px, "
                              f"off anchor)")
                        siege_fires += 1
                    except Exception as e:
                        print(f"[t_v6]  unsiege error: {e}")
        ...
        if _TECH_SPIDER_MINES in obs_tech and tgt_x is not None:
            for vult in units:
                ...
                try:
                    ...
                    print(f"[t_v6] FIRE  place_mine vulture={vid} "
                          f"@({mx},{my}) drop {drop_idx+1}/3")
                    mine_fires += 1
                except Exception as e:
                    print(f"[t_v6]  place_mine error: {e}")
        ...
        if not move_done:
            ...
            if idle:
                ...
                try:
                    ...
                    print(f"[t_v6]  cover move error: {e}")
        if move_done and not stop_done:
            ...
            if cands:
                try:
                    ...
                    print(f"[t_v6]  cover stop error: {e}")
...
def entrypoint() -> None:
    p = argparse.ArgumentParser(
        prog="python3 -m python_agent.agents.t_agent_v6",
        description="Terran v6: v5 tactics + turtle doctrine "
                    "(bunker+tank defense line, mining-corridor "
                    "placement guard, defense-first expansion).")
